jQuery(document).ready(function() {
    ...
    //Scroll To Top
    jQuery(".soufle-btn-to-top").on("click", function(){
    ...
    jQuery('.soufle-form-contact').on("submit", function(){
        ...
        var form_parent = jQuery(this).parents('.soufle-contact-form');
        $.ajax({
            type: "POST",
            cache: false,
            ...
            success: function (html) {
                ...
                form_parent.find('.soufle-contact-title').hide();
    ...
    jQuery('.soufle-form-reservation').on("submit", function(){
        ...
        var form_parent = jQuery(this).parents('.soufle-reservation-form');
        $.ajax({
            type: "POST",
            cache: false,
    ...
    jQuery(window).load(function(){
        if(screenRes > 768 ) {
            jQuery('.soufle-slider-full .item').css('height', screenHeight);
        }
    });
    ...
    // Testimonials Slider
    if(jQuery('#soufle-testimonials-slider').length > 0) {
        setTimeout(function () {
            jQuery('#soufle-testimonials-slider').carouFredSel({
                swipe: {
                    onTouch: true
                },
                pagination: "#soufle-testimonials-slider-controls",
                auto: {
                    play: true,
    ...
    jQuery(".soufle-nav-menu li.menu-item-has-children").on("hover", function(){
        var $this = $(this);
    ...
    function swapImages(){
        var active_image = $('.soufle-menu-block-hover .soufle-menu-image-array .active-image');
        var next_active_image = ($('.soufle-menu-block-hover .soufle-menu-image-array .active-image').next().length > 0) ? $('.soufle-menu-block-hover .soufle-menu-image-array .active-image').next() : $('.soufle-menu-block-hover .soufle-menu-image-array img:first');
            active_image.removeClass('active-image');
            next_active_image.addClass('active-image');
    }
    var timer;
    jQuery('.soufle-menu-category').hover(function(){
        $(this).addClass('soufle-menu-block-hover');
        timer = setInterval(swapImages, 250);
    },function(){
        clearInterval(timer);
        $(this).removeClass('soufle-menu-block-hover');
    ...
    if(jQuery('.soufle-form-reservation').length > 0){
    ...
    function photo_gallery(){
        // Masonry script
        var $gallerycontainer = jQuery('.soufle-photo-list-item').masonry();
        $gallerycontainer.masonry({
            itemSelector : '.photo'
        });
        // Sortable gallery
        jQuery('.soufle-photo-gallery-nav li a').on('click',function(){
            jQuery('.soufle-photo-gallery-nav li').removeClass('active-category');
            ...
            //if not all checked
            if(category !== 'all') {
                jQuery('.soufle-photo-list-item .photo').each(function () {
    ...
    jQuery(window).load(function(){
        if(jQuery('.soufle-photo-gallery').length > 0){
            setTimeout(function () {
                photo_gallery();
            },2);
        }
    ...
    var parallaxFunction = function(){
      jQuery('section.parallax, .soufle-slider-full .item.parallax').each(function(){
        $(this).removeClass('parallax');
      });
    };
    if(screenRes > 800){
        jQuery('.parallax').parallax("50%", "-200px", 0.1);
    }
    ...
    forms_validation('.soufle-form-reservation');
    forms_validation('.soufle-form-contact');
});
...
function start_special_offers_slider(){
    if(jQuery('#soufle-slider-special-offers').length > 0){
        jQuery('#soufle-slider-special-offers').carouFredSel({
            swipe : {
                onTouch: true
            },
...
function Mobile_Menu() {
    ...
    var mob_menu1 = jQuery(".soufle-site-navigation#soufle-menu-primary .soufle-nav-menu").clone();
    jQuery('<nav id="mobile-menu"></nav>').appendTo('.soufle-header-site .soufle-nav-wrap');
    var mob_menu = jQuery('#mobile-menu');
    mob_menu.append(mob_menu1);
    if (jQuery('.soufle-site-navigation#soufle-menu-secondary').length > 0) {
        var mob_menu2 = jQuery(".soufle-site-navigation#soufle-menu-secondary .soufle-nav-menu").clone();
        jQuery('#mobile-menu > ul > li:last-child').after(mob_menu2);
    }

    if(jQuery('.soufle-header-site.soufle-header-type-2').length > 0){
        jQuery('<a href="#mobile-menu" class="mmenu-link"><i class="fa fa-navicon"></i></a>').prependTo(".soufle-header-site .container");
    }
    else{
        jQuery('<a href="#mobile-menu" class="mmenu-link"><i class="fa fa-navicon"></i></a>').prependTo(".soufle-header-site");
    }
...
jQuery(window).resize(function() {
	...
	if(screenRes < 992){
		...
		$('.soufle-nav-wrap nav#mobile-menu').remove();
...
function sticky_menu(){
    ...
    if( jQuery('.soufle-header-site.soufle-sticky-header-on').length > 0) {
        var header_height = jQuery('.soufle-header-site').outerHeight(),
            sticky_open = header_height * 2;

        jQuery(window).on('scroll', function () {
            if (jQuery(window).scrollTop() > header_height + 5) {
                jQuery('.soufle-header-site').addClass('sticky-menu').css('top', -header_height);
            } else {
                jQuery('.soufle-header-site').removeClass('sticky-menu').css('top', 0);
            }

            if (jQuery(window).scrollTop() > sticky_open) {
                jQuery('.soufle-header-site.sticky-menu').addClass('sticky-open');
                jQuery('html').addClass('sticky-menu-opened');
            } else {
                jQuery('.soufle-header-site.sticky-menu').removeClass('sticky-open');
